...
foreach ($tests as $row) {
    ...
    $groupName = $row['test_group_name'] ?: 'Other Tests';

    $groupedTests[$groupName][] = $row;
}
...
@foreach($groupedTests as $groupName => $groupRows)

@php $sl = 1; $previousPackageGroup = null; @endphp

<tr>

<td colspan="6" style="background:#e6e6e6; font-weight:bold; color:#000;">{{ $groupName }}</td>
...
@php
    // A test with a configured analyte breakdown only needs its own
    // numbered result row printed if staff actually entered one at the
    // test level; otherwise its name is enough as a plain section label
    // above the analyte rows, avoiding a noisy blank result line.
