...
class ItemWiseSummaryReportController extends Controller
{
    ...
    */
    ...
    public function index()
    {
        ...
        return view('apps-item-wise-summary-report', compact('itemMasters'));
    }

    /*
    |--------------------------------------------------------------------------
    | SUMMARY (every item_code billed within the period)
    |--------------------------------------------------------------------------
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | DETAIL (one row per invoice, for a single item_code)
    |--------------------------------------------------------------------------
    */
    ...
    public function printDetail(Request $request)
    {
        ...
        $pdf = Pdf::loadView('apps-item-wise-summary-report-detail-pdf', [
            'rows' => $result['rows'],
            'grandTotal' => $result['grand_total'],
        ...
        return $pdf->stream(
            'Item-Wise-Summary-Report-Detail-' . $request->item_code . '-' .
            Carbon::parse($request->from_date)->format('d-m-Y') . '-to-' .
            Carbon::parse($request->to_date)->format('d-m-Y') . '.pdf'
        );
    }
    ...
    */
    ...
    private function accumulateReceivableAndSettled(array &$totals, string $fromDate, string $toDate): void
    {
        ...
        // off invoices, same as ItemWiseReportController's DOC001 special case.
    ...
    */
        ...
        if ($itemCode === self::DOCTOR_VISIT_ITEM_CODE) {
            ...
            return DB::table('invoices')
                ...
                ->get([
                    ...
                    'invoices.due_amount', 'invoices.payment_mode', 'invoices.invoice_type',
                ]);
        }
    ...
    private function decorateDetailRow($row)
    {
        ...
        $row->is_settled = (float) $row->due_amount <= 0;
