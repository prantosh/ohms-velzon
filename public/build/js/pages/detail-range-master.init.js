...
async function loadDetailRanges(page = 1) {
    currentPage = page;

    const response = await fetch(
        `/detail-range-master/list?page=${page}&per_page=${perPage}&search=${encodeURIComponent(searchTerm)}`
    );

    const result = await response.json();

    let tbody = document.querySelector('#detailRangeTable tbody');
...
document.querySelector('.tablelist-form').addEventListener('submit', async function (e) {
    ...
    let url = '/detail-range-master/store';

    if (editId) {
        url = `/detail-range-master/update/${editId}`;
    }
    ...
    if (!response.ok || !result.status) {
        ...
        let errorText = result.errors
            ...
            : (result.message ?? 'Unable to save detail range.');
...
document.getElementById('showModal').addEventListener('hidden.bs.modal', function () {
    ...
    document.querySelector('#modal-title').innerText = 'Add Detail Range';
    document.querySelector('#add-btn').innerText = 'Save Detail Range';
});

document.addEventListener('click', async function (e) {

    if (e.target.closest('#prevPage')) {
        if (currentPage > 1) loadDetailRanges(currentPage - 1);
        return;
    }

    if (e.target.closest('#nextPage')) {
        if (currentPage < lastPage) loadDetailRanges(currentPage + 1);
        return;
    }
    ...
    if (editBtn) {
        ...
        document.querySelector('#modal-title').innerText = 'Update Detail Range';
        document.querySelector('#add-btn').innerText = 'Update Detail Range';

        let id = editBtn.dataset.id;

        document.querySelector('#edit-id').value = id;

        const response = await fetch(`/detail-range-master/edit/${id}`);
        const result = await response.json();
    ...
    if (deleteBtn) {
        ...
        let confirm = await Swal.fire({
            title: 'Delete this Detail Range?',
        ...
        let response = await fetch(`/detail-range-master/delete/${id}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': csrfToken()
            }
