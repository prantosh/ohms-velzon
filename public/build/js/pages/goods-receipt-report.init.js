...
function renderDetail(result, params) {
    ...
    result.rows.forEach(row => {
        tbody.innerHTML += `
        <tr>
            <td class="fw-semibold">${escapeHtml(row.receipt_no)}</td>
            <td>${escapeHtml(row.receipt_date_fmt)}</td>
    ...
    printBtn.href = `/goods-receipt-report/print-detail?${params.toString()}`;
...
function renderSummary(result, params) {
    ...
    printBtn.href = `/goods-receipt-report/print-summary?${params.toString()}`;
    printBtn.style.display = 'inline-block';
}
...
async function loadReport() {
    ...
    const [detailResponse, summaryResponse] = await Promise.all([
        fetch(`/goods-receipt-report/detail?${params.toString()}`),
        fetch(`/goods-receipt-report/summary?${params.toString()}`),
    ]);
