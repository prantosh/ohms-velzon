...
function renderSummary(result, params) {
    ...
    document.getElementById('summary-count').innerText = g.count;
    ...
    result.rows.forEach(row => {
        ...
        <tr>
            ...
            <td class="text-end">${fmtMoney(row.gross)}</td>
    ...
    <tr class="table-light fw-bold">
        ...
        <td class="text-end">${fmtMoney(g.gross)}</td>
    ...
    printBtn.href = `/doctor-wise-payment-report/print-summary?${params.toString()}`;
...
function renderDetail(result, params) {
    ...
    document.getElementById('detail-count').innerText = g.count;
    ...
    result.rows.forEach(row => {
        let badgeClass = statusBadgeClass[row.payment_status] || 'bg-secondary-subtle text-secondary';
        tbody.innerHTML += `
        <tr>
            ...
            <td class="text-end ${pendingClass(row.pending_amount)}">${fmtMoney(row.pending_amount)}</td>
            <td class="text-center"><span class="badge ${badgeClass}">${escapeHtml(row.payment_status)}</span></td>
            <td>${escapeHtml(row.settlement_display)}</td>
    ...
    printBtn.href = `/doctor-wise-payment-report/print-detail?${params.toString()}`;
    printBtn.style.display = 'inline-block';
}
...
async function loadReport() {
    ...
    const [detailResponse, summaryResponse] = await Promise.all([
        fetch(`/doctor-wise-payment-report/detail?${params.toString()}`),
        fetch(`/doctor-wise-payment-report/summary?${params.toString()}`),
    ]);
