...
/*
|--------------------------------------------------------------------------
| RICH-TEXT EDITOR -- Microscopy content
|--------------------------------------------------------------------------
*/
...
document.querySelector('.tablelist-form').addEventListener('submit', async function (e) {
    ...
    try {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: err.message || 'Unable to save microscopy.'
        });
    }
});
...
document.addEventListener('click', async function (e) {
    ...
    if (editBtn) {
        ...
        const { result } = await fetchJson(`/microscopy-master/edit/${id}`);
    ...
    if (deleteBtn) {
        ...
        const { result } = await fetchJson(`/microscopy-master/delete/${id}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': csrfToken()
            }
