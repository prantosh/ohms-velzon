...
async function loadCategories(page = 1) {
    ...
    const response = await fetch(
        `/expenditure-category/list?page=${page}&per_page=${perPage}&search=${encodeURIComponent(searchTerm)}`
    );
    ...
    if (!result.data.length) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted">No expenditure categories found.</td></tr>';
        return;
    }
...
document.querySelector('.tablelist-form').addEventListener('submit', async function (e) {
    ...
    let url = editId ? `/expenditure-category/update/${editId}` : '/expenditure-category/store';
    ...
    if (!response.ok || !result.status) {
        ...
        let errorText = result.errors
            ...
            : (result.message ?? 'Unable to save expenditure category.');
...
document.getElementById('showModal').addEventListener('hidden.bs.modal', function () {
    ...
    document.querySelector('#modal-title').innerText = 'Add Expenditure Category';
...
document.addEventListener('click', async function (e) {
    ...
    if (editBtn) {
        ...
        document.querySelector('#modal-title').innerText = 'Update Expenditure Category';
    ...
    if (deleteBtn) {
        ...
        let confirm = await Swal.fire({
            title: 'Delete Expenditure Category?',
        ...
        let response = await fetch(`/expenditure-category/delete/${id}`, {
            method: 'DELETE',
