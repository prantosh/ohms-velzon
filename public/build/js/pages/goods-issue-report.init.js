...
function renderDetail(result, params) {
    ...
    result.rows.forEach(row => {
        tbody.innerHTML += `
        <tr>
            <td class="fw-semibold">${escapeHtml(row.issue_no)}</td>
            <td>${escapeHtml(row.issue_date_fmt)}</td>
            <td>${escapeHtml(row.issued_to_name) || '-'}</td>
            <td>${escapeHtml(row.item_code)}</td>
            <td>${escapeHtml(row.item_name)}</td>
            <td class="text-center">${escapeHtml(row.uom)}</td>
            <td class="text-end">${fmtQty(row.issue_qty)}</td>
    ...
    printBtn.href = `/goods-issue-report/print-detail?${params.toString()}`;
...
function renderSummary(result, params) {
    ...
    printBtn.href = `/goods-issue-report/print-summary?${params.toString()}`;
    printBtn.style.display = 'inline-block';
}
...
async function loadReport() {
    ...
    const [detailResponse, summaryResponse] = await Promise.all([
        fetch(`/goods-issue-report/detail?${params.toString()}`),
        fetch(`/goods-issue-report/summary?${params.toString()}`),
    ]);
