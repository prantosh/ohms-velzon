...
async function loadAgencies(page = 1) {
    ...
    const response = await fetch(
        `/expenditure-agency/list?page=${page}&per_page=${perPage}&search=${encodeURIComponent(searchTerm)}`
    );

    const result = await response.json();

    let tbody = document.querySelector('#agencyTableBody');
    ...
    if (!result.data.length) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted">No expenditure agencies found.</td></tr>';
        return;
    }
...
document.querySelector('.tablelist-form').addEventListener('submit', async function (e) {
    ...
    let url = editId ? `/expenditure-agency/update/${editId}` : '/expenditure-agency/store';
    ...
    if (!response.ok || !result.status) {
        ...
        let errorText = result.errors
            ...
            : (result.message ?? 'Unable to save expenditure agency.');
...
document.getElementById('showModal').addEventListener('hidden.bs.modal', function () {
    ...
    document.querySelector('#modal-title').innerText = 'Add Expenditure Agency';
    document.querySelector('#add-btn').innerText = 'Save Agency';
});

document.addEventListener('click', async function (e) {

    if (e.target.closest('#prevPage')) {
        if (currentPage > 1) loadAgencies(currentPage - 1);
        return;
    }

    if (e.target.closest('#nextPage')) {
        if (currentPage < lastPage) loadAgencies(currentPage + 1);
        return;
    }
    ...
    if (editBtn) {
        ...
        document.querySelector('#modal-title').innerText = 'Update Expenditure Agency';
        document.querySelector('#add-btn').innerText = 'Update Agency';
    ...
    if (deleteBtn) {
        ...
        let confirm = await Swal.fire({
            title: 'Delete Expenditure Agency?',
        ...
        let response = await fetch(`/expenditure-agency/delete/${id}`, {
            method: 'DELETE',
        ...
        if (result.status) loadAgencies(currentPage);
    }
});
