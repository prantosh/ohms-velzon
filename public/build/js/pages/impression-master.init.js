...
/*
|--------------------------------------------------------------------------
| RICH-TEXT EDITOR -- Impression content
|--------------------------------------------------------------------------
*/
...
async function loadImpressions(page = 1) {
    currentPage = page;

    const { result } = await fetchJson(
        `/impression-master/list?page=${page}&per_page=${perPage}&search=${encodeURIComponent(searchTerm)}`
    );

    let tbody = document.querySelector('#impressionTable tbody');
...
document.querySelector('.tablelist-form').addEventListener('submit', async function (e) {
    ...
    try {
        ...
        let url = '/impression-master/store';

        if (editId) {
            url = `/impression-master/update/${editId}`;
        }
        ...
        if (!response.ok || !result.status) {
            ...
            let errorText = result.errors
                ...
                : (result.message ?? 'Unable to save impression.');
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: err.message || 'Unable to save impression.'
        });
    }
});
...
document.getElementById('showModal').addEventListener('hidden.bs.modal', function () {
    ...
    document.querySelector('#modal-title').innerText = 'Add Impression';
    document.querySelector('#add-btn').innerText = 'Save Impression';
});

document.addEventListener('click', async function (e) {

    if (e.target.closest('#prevPage')) {
        if (currentPage > 1) loadImpressions(currentPage - 1);
        return;
    }

    if (e.target.closest('#nextPage')) {
        if (currentPage < lastPage) loadImpressions(currentPage + 1);
        return;
    }
    ...
    if (editBtn) {
        ...
        document.querySelector('#modal-title').innerText = 'Update Impression';
        document.querySelector('#add-btn').innerText = 'Update Impression';

        let id = editBtn.dataset.id;

        document.querySelector('#edit-id').value = id;

        const { result } = await fetchJson(`/impression-master/edit/${id}`);
    ...
    if (deleteBtn) {
        ...
        const { result } = await fetchJson(`/impression-master/delete/${id}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': csrfToken()
            }
