...
async function loadKits(page = 1) {
    currentPage = page;

    const response = await fetch(
        `/kit-master/list?page=${page}&per_page=${perPage}&search=${encodeURIComponent(searchTerm)}`
    );

    const result = await response.json();

    let tbody = document.querySelector('#kitTable tbody');
...
document.querySelector('.tablelist-form').addEventListener('submit', async function (e) {
    ...
    let url = '/kit-master/store';

    if (editId) {
        url = `/kit-master/update/${editId}`;
    }
    ...
    if (!response.ok || !result.status) {
        ...
        let errorText = result.errors
            ...
            : (result.message ?? 'Unable to save kit.');
...
document.getElementById('showModal').addEventListener('hidden.bs.modal', function () {
    ...
    document.querySelector('#modal-title').innerText = 'Add Kit';
    document.querySelector('#add-btn').innerText = 'Save Kit';
});

document.addEventListener('click', async function (e) {

    if (e.target.closest('#prevPage')) {
        if (currentPage > 1) loadKits(currentPage - 1);
        return;
    }

    if (e.target.closest('#nextPage')) {
        if (currentPage < lastPage) loadKits(currentPage + 1);
        return;
    }
    ...
    if (editBtn) {
        ...
        document.querySelector('#modal-title').innerText = 'Update Kit';
        document.querySelector('#add-btn').innerText = 'Update Kit';

        let id = editBtn.dataset.id;

        document.querySelector('#edit-id').value = id;

        const response = await fetch(`/kit-master/edit/${id}`);
        const result = await response.json();
    ...
    if (deleteBtn) {
        ...
        let response = await fetch(`/kit-master/delete/${id}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': csrfToken()
            }
